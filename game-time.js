...
module.exports = gameTime = ({
	...
	delimeterDay: 	 		[// Текстовое представление Времени суток
		'ночь',
		'утро',
		'день',
		'вечер'
	...
	secondArr: 		 			{// Состав timestamp в секундах
		seconds: 0,
		minute:	 60,
		hour:		 3600,
		day:		 86400,
	...
	setTimeNow(timestamp) {// Устанавливаем timestamp из базы в наш код
		gameTime.nowTime = timestamp
	},
	setBD(command='') {// Запрос и Сохранение в Базу timestamp
		const setTimeNow = this.setTimeNow
		if (command == 'update'){
			con.query('UPDATE variables SET timeStamp = ?', [this.nowTime], function (error, results, fields) {
				if (error) throw error;
			});
		} else if (command == 'select'){
			con.query('SELECT * FROM variables', function (error, results, fields) {
				if (error) throw error;
				setTimeNow(parseInt(results[0].timeStamp))
			})
		}
	},
	init(){// Инициализация таймера времени
		(this.nowTime === 0 ? (this.nowTime += this.startTime) : null);
		(!this.reverseTimestamp ? (gameTime.nowTime += 1) : (gameTime.nowTime -= 1));
		this.saveDBtimer -= 1
		...
		if (typeof this.refreshDB == 'boolean') {
			this.setBD('select')
			console.log('select');
		}

		if (this.saveDBtimer === 0) {
			this.refreshDB = setTimeout(() => {
				this.saveDBtimer = this.speedOfUpdate
				this.setBD('update')
			}, this.saveDBtimer * this.speedOfTime)
		}
